...
<script>
    ...
    function crearCanales(canales, contador) {
        ...
        var counter = 0;
        var operadores = listarOperadores();
        var tecnologias = listarTecnologias();
        var tecnologias2 = listarTecnologiasPorOperador(1);
        console.log("canales canales ", canales);
        for (; contador < canales; contador++) {
            var col_canal = '<tr class="ranura_' + (Math.floor(contador / 4) + 1) + '" id="row_' + (contador + 1) + '" class="canales_temporales"><td>Canal ' + (contador + 1) + '</td>';

            var col_operador = '<td><select name="op_' + contador + '" class="form-control select_operador"  id="op_' + contador +'">'; //<option value="">Movistar del Perú</option><option value="">America Movil</option></select></td>';
            ...
            var col_tecnologia = '<td><select name="tec_' + contador + '" class="form-control"id="tec_' + contador +'">';
            col_tecnologia += '<option value="">Seleccione</option>'
   
            col_tecnologia += '</select></td>';

            var col_slot = '<td class="slot_' + (Math.floor(contador / 4) + 1) + '" id="Slot_' + (contador + 1) + '">Slot ' + (Math.floor(contador / 4) + 1) + '</td>';

            var col_numeros = '<td><input type="text" id="numero" name="numero" class="form-control m-b-5"></td>';

            var col_opciones = '<td><button class="btn btn-danger btn-eliminar"><i class="fa fa-trash-o"></i>Eliminar</button></td></tr>';

            var tableBody = $("#tabla_canales tbody");
    ...
    function cargarCanales(canales, response) {
        ...
        for (i = 0; i < canales; i++) {
            var id_canal = response.equipo_response.canales[i].id_canal;
            var col_canal = '<tr class="ranura_' + (Math.floor(i / 4) + 1) + '" id="row_' + (i + 1) + '"><td>Canal ' + (i + 1) + '<input name="id_canal" type="hidden" value="'+id_canal+'"></td>';

            var col_operador = '<td><select name="op_' + i + '" id="op_' + i +'" class="form-control" >'; //<option value="">Movistar del Perú</option><option value="">America Movil</option></select></td>';
            operadores.forEach(operador => {
                col_operador += '<option value="' + operador.id_operador_telefonico + '">' + operador.nombre + '</option>';
            });
            col_operador += '</select></td>';

            var col_tecnologia = '<td><select name="tec_' + i + '" id="tec_' + i +'" class="form-control">';
            tecnologias.forEach(tecnologia => {
                col_tecnologia += '<option value="' + tecnologia.id_tecnologia + '">' + tecnologia.nombre + '</option>';
            });
            col_tecnologia += '</select></td>';

            var col_slot = '<td class="slot_' + (Math.floor(i / 4) + 1) + '" id="Slot_' + (i + 1) + '">Slot ' + response.equipo_response.canales[i].nro_ranura + '</td>';

            var col_numeros = '<td><input type="text" id="numero" name="numero" value="' + response.equipo_response.canales[i].numero + '" class="form-control m-b-5"></td>';
            ...
            $('select[name="op_' + i + '"]').val(response.equipo_response.canales[i].id_operador);
            $('select[name="tec_' + i + '"]').val(response.equipo_response.canales[i].id_tecnologia);
            console.log("oper", response.equipo_response.canales[i].id_tecnologia);
            console.log("tec_", response.equipo_response.canales[i].id_tecnologia);
        }
